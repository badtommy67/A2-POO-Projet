...
// POUR COMPRENDRE LE FONCTIONNEMENT DE CETTE GRILLE, IL FAUT SE REFERER A GRILLE_TORIQUE.CPP QUI COMPORTE LES MEMES EXPLICATIONS
...
int Grille::compterVoisins(int x, int y) {
    int nb_voisins = 0;
    for (int i = std::max(0, x - 1); i <= std::min(ligne - 1, x + 1); i++) { // ici le calcul de voisin est plus simple, on regarde les 8 cellules autour
